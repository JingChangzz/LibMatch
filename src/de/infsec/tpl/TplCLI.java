...
public class TplCLI {
	...
	public static void main(String[] args){
		String androidjar = "D:/Android/platforms/android-19/android.jar";
		String libxmlPath = "";
		String jarFile = "";
		HashMap<String, String> inputs = new HashMap<>();

		String rootDir = "E:/gradute/libs-ILC";
		for (File f1 : new File(rootDir).listFiles()){ // cate
			for (File f2 : f1.listFiles()){  //name
				for (File f3 : f2.listFiles()){ //version
					for (File f4 : f3.listFiles()){ // xml and jar
						if (f4.getName().endsWith(".xml")){
							libxmlPath = f4.getAbsolutePath();
						}else if (f4.getName().endsWith("jar") || f4.getName().endsWith(".aar")){
							jarFile = f4.getAbsolutePath();
						}
					}
					inputs.put(libxmlPath, jarFile);
				}
			}
		}
		...
		while (iterator.hasNext()){
			...
			libxmlPath = (String) entry.getKey();
			jarFile = (String) entry.getValue();
			String[] command = new String[]{"-o", "profile", "-a", androidjar, "-x", libxmlPath, jarFile};
			mainT(command);
			i++;
			System.out.println(jarFile + "finished..." + i+"/" + inputs.size());
		}
		String apkDir = "D:/Desktop/apkCrewler/APKCrawler-master/baidu";
		for (File a : new File(apkDir).listFiles()){
			String[] commandMatch = new String[]{"-o", "match", "-a", androidjar, "-p", "E:/gradute/LibScout-master/profiles",
					"-s", "-d", "E:/gradute/LibScout-master/logging", a.getAbsolutePath()};
			mainT(commandMatch);
		}
		String[] commandDB = new String[]{"-o", "db", "-p", "E:/gradute/LibScout-master/profiles", "-s","E:/gradute/LibScout-master/stats"};
		mainT(commandDB);

	}

	public static void mainT(String[] args) {
//		args = new String[]{"-o", "db", "-p", "E:/gradute/LibScout-master/profiles", "-s"};
